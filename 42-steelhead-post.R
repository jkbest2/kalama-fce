...
eff_df |>
  ...
  ggplot(aes(x = date, y = log_discharge_scl_eff, ymin = .lower, ymax = .upper)) +
  geom_hline(yintercept = 0, alpha = 0.25, linetype = "dashed") +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
  eff_df |>
  point_interval(temperature_scl_eff) |>
  ggplot(aes(x = date, y = temperature_scl_eff, ymin = .lower, ymax = .upper)) +
  geom_hline(yintercept = 0, alpha = 0.25, linetype = "dashed") +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
  eff_df |>
  point_interval(`log_discharge_scl:temperature_scl_eff`) |>
  ggplot(aes(x = date, y = `log_discharge_scl:temperature_scl_eff`, ymin = .lower, ymax = .upper)) +
  geom_hline(yintercept = 0, alpha = 0.25, linetype = "dashed") +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
  plot_layout(ncol = 1)
...
eff_df |>
  point_interval(log_discharge_scl_eff) |>
  ggplot(aes(x = discharge, y = log_discharge_scl_eff, ymin = .lower, ymax = .upper)) +
  geom_hline(yintercept = 0, alpha = 0.25, linetype = "dashed") +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
  eff_df |>
  point_interval(temperature_scl_eff) |>
  ggplot(aes(x = temperature, y = temperature_scl_eff, ymin = .lower, ymax = .upper)) +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
  # eff_df |>
  # point_interval(`log_discharge_scl:temperature_scl_eff`) |>
  # ggplot(aes(x = discharge * temperature, y = `log_discharge_scl:temperature_scl_eff`, ymin = .lower, ymax = .upper)) +
  # geom_ribbon(alpha = 0.4) +
  # geom_line() +
